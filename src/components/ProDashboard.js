...
const ProDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState('actions'); // 'actions' or 'visualizer'
  ...
  const handleManualCrisis = (e) => {
    ...
    setPortfolio(prev => {
      ...
      // Deduct from safe debt first, then bleed into equity if debt hits 0
      if (newDebt < 0) {
        newEquity += newDebt; 
        newDebt = 0;
      }
  ...
  const getHistoricalData = () => {
    ...
    const currentSpentTotal = Object.values(expenses).reduce((a, b) => a + b, 0) + totalEmi;
    ...
    return {
      labels,
      datasets: [
        { label: 'Outflow (Spent + Debt)', data: spentData, backgroundColor: '#ef4444', borderRadius: 4 },
        { label: 'Capital Deployed', data: investData, backgroundColor: '#8b5cf6', borderRadius: 4 }
      ]
    };
  };

  const getCorpusData = () => {
    let labels = []; let predicted = [];
    ...
    return { labels, datasets: [{ label: 'Predicted Net Worth', data: predicted, borderColor: '#8b5cf6', backgroundColor: 'rgba(139, 92, 246, 0.1)', fill: true, tension: 0.4 }] };
  };
    ...
    <div className="dash-container">
      <header className="dash-header">
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <span style={{ fontWeight: 'bold', color: '#8b5cf6' }}>{xp} XP</span>
        </div>
      </header>
          ...
          <>
            ...
            <div className="dash-col" style={{ animation: 'fadeIn 0.3s' }}>
              <div style={{ backgroundColor: '#111827', padding: '1.5rem', borderRadius: '1rem', border: '1px solid #374151' }}>
                ...
                <div style={{ height: '300px' }}><Line data={getCorpusData()} options={{ responsive: true, maintainAspectRatio: false }} /></div>
              </div>
            </div>
          </>
        )}
